...
function* createBranchParagraph() {
  ...
  //       expanded: true,
  //       ranges: [
  //           yield createRange(20),
  //           yield createRange(50),
  //           yield createRange(80),
  //           yield createRange(),
  //       ]
  //   };
}
...
function* insertBranchParagraph(action) {
  ...
  // const parent = yield getParagraph(parent_id);
  // const child = yield createBranchParagraph();
  // child.ranges[0].next_id = getParagraphNextId(parent, branch_index);
  // child.chat_id = parent.chat_id;
  // yield updateParagraph(setParagraphNextId(parent, branch_index, child.id), child);
  //
  // return { type: 'Insert', parent_id, branch_index, child_paragraph: null };
...
export function* requestProject(action) {
  try {
    ...
    const result = yield Api.fetch(config.url + '/v1/project/' + action.id, {
      method: 'GET',
  ...
  try {
    ...
    const upload = (file) => new Promise((resolve, reject) => {
      ...
      if (type === '.jpg' || type === '.jpeg' || type === '.png' || type === '.gif') {
        if (action.filetype === 'img' || action.filetype === 'both') {
        ...
        if (action.filetype === 'video') {
          ...
          reject('文件格式不正确，图片支持.jpg、.jpeg、.gif、.png等文件格式，视频仅支持.mp4格式，语音仅支持.mp3格式！');
        }
      }
    });
...
function* commitProject(action) {
  try {
    yield setAppMessage('开始验证剧本');
    yield validate();
    const errors = yield getErrors();
    if (errors.length > 0) {
      yield setAppAlert('发布剧本失败: 剧本存在错误', null);
    } else {
      yield setAppMessage('开始编译剧本');
      const outline = yield getOntline();
      const content = yield getContent();
      const script = buildProject(outline, content);
      const lock_paragraphs = getLockParagraphs(content, outline.id);
      yield put({
        type: 'UPDATE_PROJECT_OUTLINE',
        outline: {
          ...outline,
          script: JSON.stringify(script),
          lock_paragraphs: JSON.stringify(lock_paragraphs),
        }
      });
      yield setAppMessage('编译剧本完成');
      const result = yield saveProject();
      if (result) {
        const token = yield getToken();
        yield setAppLoading('正在投稿......');
        yield Api.fetch(config.url + '/v1/project/commit/' + outline.id, {
          method: 'GET',
          headers: { "Content-Type": "application/json", "Authorization": token },
        });
        yield setAppMessage('投稿成功，请耐心等待审核结果');
      } else {
        yield setAppMessage('剧本保存失败，请手动保存后再次发布！');
      }
    }
  } catch (e) {
    if (e instanceof AppError) {
